...
import { CreditCard, RefreshCw, AlertCircle, DollarSign, Calendar, Hash, ExternalLink, CheckCircle, Clock, TrendingUp, Database } from 'lucide-react';
...
const PayoutStatsSection: React.FC = () => {
  ...
  const calculateTotals = () => {
    return payments.reduce((acc, payment) => ({
      ...
      completedPayments: acc.completedPayments + (payment.completed ? 1 : 0),
      pendingPayments: acc.pendingPayments + (!payment.completed && !payment.canceled ? 1 : 0),
    }), {
      totalPayouts: 0,
      totalBytes: 0,
      completedPayments: 0,
    ...
    <div className="space-y-8">
      ...
      {error && (
        <div className="bg-red-900/50 border border-red-700 p-4 rounded-xl flex items-start gap-3">
          ...
          <div>
            <h3 className="font-medium text-red-300">Error loading payout data</h3>
            ...
            <div className="bg-gradient-to-r from-gray-700 to-gray-800 px-6 py-4 border-b border-gray-600">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-gray-300 mt-1">History of payouts and earnings</p>
                </div>
                <div className="bg-gray-600 text-gray-200 px-3 py-1 rounded-lg border border-gray-500 text-sm">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-700">
                <thead className="bg-gray-900">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Data Transferred</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Payment Time</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Transaction</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Blockchain</th>
                  </tr>
                </thead>
                <tbody className="bg-gray-800 divide-y divide-gray-700">
                  {payments.map((payment, index) => (
                    <tr key={payment.payment_id} className={`hover:bg-gray-700/50 transition-colors ${index === 0 ? 'bg-green-900/20 border-l-4 border-green-500' : ''}`}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-300">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-xs">
                            {payment.tx_hash.substring(0, 8)}...{payment.tx_hash.substring(-8)}
                          </span>
                          <a
              ...
              {payments.length === 0 && !isLoading && (
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-200 mb-2">No Payments Found</h3>
                  <p className="text-gray-400 italic">No payout data available. Try refreshing the data.</p>
                </div>
              )}
            </div>
          </div>
