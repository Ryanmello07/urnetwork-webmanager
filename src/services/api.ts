...
export const fetchClients = async (token: string): Promise<ClientsResponse> => {
  try {
    console.log('Fetching clients with token:', token ? 'Token present' : 'No token');
    ...
    if (!response.ok) {
      console.error('Fetch clients failed:', response.status, response.statusText);
      const errorData = await response.text();
      console.error('Error response:', errorData);
      throw new Error(`HTTP error! status: ${response.status}`);
    }
...
// Remove client API
export const removeClient = async (token: string, clientId: string): Promise<RemoveClientResponse> => {
  try {
    ...
    if (!response.ok) {
      console.error('Remove client failed:', response.status, response.statusText);
      const errorData = await response.text();
      console.error('Error response:', errorData);
      throw new Error(`HTTP error! status: ${response.status}`);
    }
    ...
    return {
      error: {
        message: error instanceof Error ? error.message : 'Failed to remove client',
      },
    };
  }
};
...
export const fetchProviderStats = async (token: string): Promise<StatsResponse> => {
  try {
...
// Get leaderboard API
export const fetchLeaderboard = async (token: string): Promise<LeaderboardResponse> => {
  try {
...
export const fetchNetworkRanking = async (token: string): Promise<NetworkRanking> => {
  try {
    ...
    return {
      ...
      error: {
        message: error instanceof Error ? error.message : 'Failed to fetch network ranking',
      },
    };
  }
};
...
export const fetchProviderLocations = async (): Promise<ProviderLocationsResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL}/network/provider-locations`, {
      method: 'GET',
      headers: {
        'Accept': '*/*',
      },
    ...
    return {
      ...
      devices: [],
      error: {
        message: error instanceof Error ? error.message : 'Failed to fetch provider locations',
      },
    };
  }
};
...
export const findProviderLocations = async (query: string): Promise<ProviderLocationsResponse> => {
  try {
    const response = await fetch(`${API_BASE_URL}/network/find-provider-locations`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    return {
      ...
      devices: [],
      error: {
        message: error instanceof Error ? error.message : 'Failed to find provider locations',
      },
    };
  }
};
...
export const fetchWalletStats = async (token: string): Promise<WalletStatsResponse> => {
  try {
    ...
    return {
      ...
      error: {
        message: error instanceof Error ? error.message : 'Failed to fetch wallet stats',
      },
    };
  }
};
